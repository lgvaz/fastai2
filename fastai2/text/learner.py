...
class LMLearner(TextLearner):
    ...
    def predict(self, text, n_words=1, no_unk=True, temperature=1., min_p=None, no_bar=False,
        ...
        pad_idx = self.dls.vocab.index(PAD)
        ...
        for _ in (range(n_words) if no_bar else progress_bar(range(n_words), leave=False)):
            ...
            samples = torch.multinomial(res, 1).to(idxs.device)
            idxs = idxs_all = retain_type(torch.cat([idxs_all, samples], dim=-1), idxs)
            if only_last_word: idxs = idxs[:, -1:]
        vocab = self.dls.train_ds.numericalize.vocab
        tokens = [[vocab[i] for i in row if vocab[i] not in [BOS, PAD]] for row in idxs_all]
        sep = self.dls.train_ds.tokenizer[-1].sep
        out = [sep.join(decoder(o)) for o in tokens]
